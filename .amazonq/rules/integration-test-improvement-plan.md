...
### Phase 1完了時
- [ ] テストファイルが適切に分類・移動されている
- [ ] 既存のテストが実行可能（スキップ含む）

### Phase 2完了時
- [ ] CIの `integration` ジョブが成功する
- [ ] CIの `bats-test` ジョブが成功する
- [ ] 全体のCIパイプラインがグリーンになる
...
1. **このドキュメントのレビュー**: 計画内容の妥当性確認
2. **Phase 1の実装開始**: テストファイルの分類と移動
3. **Phase 2の実装**: CIワークフロー修正
4. **動作確認**: ローカルおよびCI環境でのテスト実行

---

**作成日**: 2025-01-XX（CI/ビルド時に自動埋め込み）
**最終更新**: 2025-01-XX（CI/ビルド時に自動埋め込み）
**ステータス**: 📋 計画策定中
