...
## 19. Phase 1 完了サマリー（2025-10-15）

### 達成事項

✅ **コア実装**
- `ActBridgeRunner` の完全実装（act CLI呼び出し、リトライ、フォールバック）
- `SimulationService` への統合とFeature Flag (`ACTIONS_USE_ACT_BRIDGE`)
- エラーハンドリングとログ出力の整備

✅ **テスト基盤**
- `pytest.mark.legacy_actions` マーカーの導入
- ユニットテストでのブリッジモード動作確認（17ファイル対象）
- 既存テストの互換性維持

✅ **ドキュメント**
- コマンド使い分けガイドの更新（`make actions-ci` vs `make actions-run`）
- 移行計画書の進捗記録
- ユーザー向け警告と移行情報の追加

✅ **検証**
- `act` コマンドの動作確認（ワークフロー一覧、ドライラン成功）
- フルイメージ（`ghcr.io/catthehacker/ubuntu:full-24.04`）での実行確認
- ブリッジモードでのユニットテスト成功

### 残課題（Phase 2へ）

🔄 **テスト移行**
- 17個のテストファイルに `legacy_actions` マーカーを段階的に付与
- BATS/E2Eテストの完全なact対応
- テストカバレッジの維持

🔄 **CI/CD統合**
- Nightly CIでの `make actions-ci` サニティチェック
- ブリッジモードメトリクスの収集と可視化
- 失敗時の自動通知

🔄 **機能補完**
- 診断機能の代替実装（Docker/act検証）
- ハング検知の代替手段
- アーティファクト出力の最適化
...
### 目標
- BATS/E2Eテストを `act` 直接実行へ完全移行
- `services/actions` 依存をゼロへ削減
- 代替機能（診断・ハング検知）の提供

### チケット案

| チケットID | タスク概要 | 見積もり | 優先度 |
| --- | --- | --- | --- |
| ACT-201 | ユニットテスト17ファイルに `legacy_actions` マーカー付与と移行判断 | 3日 | 高 |
| ACT-202 | BATS テストの act 直接実行への書き換え | 5日 | 高 |
| ACT-203 | E2Eテストの act 対応と統合テスト更新 | 5日 | 高 |
| ACT-204 | 診断機能の代替実装（`scripts/`配下に独立ツール化） | 3日 | 中 |
| ACT-205 | ハング検知の代替手段検討（タイムアウト設定等） | 2日 | 中 |
| ACT-206 | アーティファクト出力の最適化（act標準出力からの抽出） | 2日 | 低 |
| ACT-207 | CI/CD統合とメトリクス収集 | 3日 | 中 |
| ACT-208 | `services.actions` import禁止のリンター追加 | 1日 | 高 |
