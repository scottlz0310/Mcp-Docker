...
Phase 0 → Phase 1 → Phase 2 → Phase 3 → Phase 4 → Phase 5 → Phase 6
                ↑        │ (0件)                               ↓ WAIT / REQUEST_REREVIEW
                └────────┼────────────────────────────────────┘
                         ↓ または READY_TO_MERGE（Phase 6 から）
...
**① サブエージェントへの委譲（推奨）**  
エージェント環境がサブエージェントの生成をサポートしている場合:

1. 以下の情報を渡してサブエージェントを生成し、ポーリングループ全体を委譲する:
   - `watch_id`（`start_copilot_review_watch` レスポンスから）
   - `owner` / `repo` / `pr`
   - タスク: `next_poll_seconds` 秒待機 → `{CRM}:get_copilot_review_watch_status` 呼び出し → `recommended_next_action` が `POLL_AFTER` なら待機・再ポーリング、それ以外は即終了してメインエージェントに最終アクションを返す
2. サブエージェントが完了するまで同期的に待機し、最終 `recommended_next_action` を受け取る。
3. 下表に従って次フェーズへ進む。

> **サブエージェント委譲のメリット**: ポーリングループをサブコンテキストに隔離することで、
> メインコンテキストの汚染を防ぎ、後続フェーズの推論精度を維持できる。

**② メインエージェントによる直接ポーリング（フォールバック）**  
サブエージェントが利用できない場合:

- レスポンスの `next_poll_seconds` 秒後に `{CRM}:get_copilot_review_watch_status` を呼ぶ（最小 1 秒、サーバー既定は 90 秒）。

いずれの場合も `recommended_next_action` に従う:
...
- スレッドが 0 件: Copilot が「問題なし」と判断した可能性 → Phase 6.5 へ（CI / カバレッジ確認）。
...
1. `git status` で作業ツリーの現在状態（未コミット変更・ステージ済みファイル）を確認してから着手する。
2. `accept` した項目のみ修正する。
3. **修正粒度**: 1 スレッド = 1 論理変更単位（atomic）。別スレッドへの波及がある場合は採否テーブルを再評価してから進める。
4. 全修正完了後にビルド・テストを再実行。失敗したら修正してリトライ。解消不能な場合はサイクル中断してユーザーに報告。
5. Phase 4 完了後に**まとめて 1 コミット**する（Conventional Commits 形式）。同ブランチへの push が既に存在する場合は追加コミットとして積む（force push 禁止）。
...
> **`max_cycles` は常に `0` を渡す。エージェントはこの値を変更してはならない**（詳細は「再レビュー上限の扱い」節を参照）。  
> `cycles_done` は 0 始まりの整数（数値型）で、**Phase 2〜5 を完走したサイクル数**を表す。WAIT / REQUEST_REREVIEW が返ったらインクリメントして次サイクルへ進む（初回: `0`、2 サイクル目: `1`、…）。
...
| `REQUEST_REREVIEW` | `{CRM}:request_copilot_review` → `cycles_done` をインクリメントして Phase 1 へ戻る |
| `READY_TO_MERGE` | Phase 6.5 へ |
| `ESCALATE` | 現状をユーザーに報告してサイクル終了（人間判断に委ねる） |

### 再レビュー上限の扱い

再レビュー上限（サーバー側 `MAX_REVIEW_CYCLES`、既定 3）は**固定**である。**エージェントはこの値を変更してはならない。**

- **例外を作らない。** 「人が毎サイクル起動している（Human in the loop）だから安全」「あと 1 サイクルで収束する」といった判断で `max_cycles` に 0 以外を渡して引き上げてはならない。skill の内側から起動元が自動サイクルか人の手動起動かは判別できず、誤判定は警告も痕跡も残さずに起きる。上限がもっとも要る状況ほど、エージェントは「自分は例外だ」と判断しやすい。
- **延長は人の明示指示によってのみ発生する。** `ESCALATE` に到達した後、ユーザーが「続行」と明示的に指示した場合に限り、指示された分だけ上限を延長して Phase 6 のルーティングへ戻る。エージェントは延長を**提案**できるが、**実行はできない**。
- **上限が止めるのは「再レビュー依頼」だけである。** `{CRM}:request_copilot_review` の発行（＝自動継続のトリガー）を止めるのであって、指摘への対応を止めるものではない。`ESCALATE` に到達したサイクルでも、**指摘の分類・修正・コミット・返信・resolve は通常どおり実行する**。
- **`ESCALATE` は回避すべき失敗状態ではない。** 現状をユーザーに報告し、人がマージ可否・続行可否を判断する**正常な合流点**である。行き止まりではないため、`ESCALATE` を避けるために上限を動かす必要はない。
...
- 再レビュー上限: サーバー側 `MAX_REVIEW_CYCLES`（デフォルト 3）。**エージェントは変更できない**（`max_cycles` は常に `0` を渡す。延長は人の明示指示のみ。「再レビュー上限の扱い」節を参照）。上限が止めるのは再レビュー依頼だけで、指摘への対応は通常どおり行う。`ESCALATE` は回避すべき失敗ではなく人間判断へ合流する正常な経路
- 修正粒度: スレッド単位 atomic（1 スレッド = 1 論理変更単位）
- コミット戦略: Phase 4 完了後まとめて 1 コミット（Conventional Commits 形式）。push 済みの場合は追加コミット（force push 禁止）
