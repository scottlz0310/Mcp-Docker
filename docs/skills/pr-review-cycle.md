...
|----------------|------|---------------|
| `{CRM}` | copilot-review-mcp ツール | `mcp_copilot-revie_*` |
...
**ポーリング fallback:**
- レスポンスの `next_poll_seconds` 秒後（未指定時は **270 秒**）に `{CRM}:get_copilot_review_watch_status` を呼ぶ。
...
> `cycles_done` は Phase 1 進入ごとにインクリメントする（最初は 0）。
...
- ポーリング間隔: レスポンスの `next_poll_seconds` を優先、未指定時は 270 秒固定
...
| `wait_for_copilot_review` | ※ legacy。blocking wait。代わりに watch を使うこと |
