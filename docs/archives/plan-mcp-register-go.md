...
5. サービス名は ROUTE キーから導出（`ROUTE_GITHUB` → サジェスト名 `github`）

> **サービス名のマッピング問題**: `ROUTE_GITHUB` から `github`、`ROUTE_COPILOT_REVIEW` から `copilot-review` への変換に命名規則が必要。
