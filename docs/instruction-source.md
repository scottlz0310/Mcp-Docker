...
- `link` は、異なる symlink、壊れた symlink、通常ファイルを、確認後に同一ディレクトリへバックアップしてから置き換える。
...
- 配置先がディレクトリ、または未対応のファイル種別の場合は自動置換せず停止する。

したがって、既存ファイルがある場合にバックアップを取るのは正しいが、未確認の通常ファイルを黙って上書きすることはしない。既存の symlink もリンク自体を移動して保存するため、リンク先の内容をコピーしない。
