...
## Remote MCP プロバイダー（upstream_bearer_token_env）

mcp-gateway v0.5.0 以降、外部 Remote MCP サービス（例: Cloudflare、Supabase 等）を
ローカル Gateway 配下に収容できます。CLI には `http://127.0.0.1:8080/mcp/<name>` という
ローカル URL のみを書き、API トークンはコンテナ環境変数に閉じ込めます。

### Cloudflare Remote MCP（セットアップ例）

**1. Cloudflare API トークン取得**

[Cloudflare ダッシュボード](https://dash.cloudflare.com/profile/api-tokens) で API トークンを作成してください。
...
同じパターンで任意の Remote MCP サービスを追加できます:

```
ROUTE_<NAME>=/<prefix>|<upstream_url>|auth=oauth|upstream_bearer_token_env=<ENV_NAME>
```

| オプション | 対象経路 | 説明 |
|---|---|---|
| `auth=oauth` | client → gateway | OAuth 認証を要求 |
| `upstream_bearer_token_env=ENV_NAME` | gateway → upstream | 環境変数のトークンを Bearer として注入 |

- upstream が 401 を返してもクライアント OAuth セッションは無効化されません
- env var 未設定時はスタートアップ時にエラー終了（Fail-closed）
