...
servers:
  - name: cloudflare
    ...
    # tokenEnv: CLOUDFLARE_API_TOKEN  # Codex のみ: 静的 API トークンを使う場合に追加
```

> **注意**: `tokenEnv` を含むエントリは Claude / Copilot / Antigravity CLI では登録がスキップされます（シークレットをヘッダーとして保存しない設計のため）。これらのクライアントは `tokenEnv` なしの URL のみを登録し、OAuth フローで直接認証します。
