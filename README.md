...
## upstream OAuth 委任（upstream_oauth）

mcp-gateway main ブランチ以降、外部 Remote MCP サービス（例: Cloudflare 等）を
upstream OAuth 委任モードで Gateway 配下に収容できます。
Gateway が RFC 9728/RFC 8414 で upstream AS を自動検出し、ユーザーごとに
authorization_code + PKCE で OAuth 認可を取得します。

### Cloudflare Remote MCP（セットアップ例）

**1. ルートを有効化**

シェル環境変数または `.env` に以下を追記します（値は on/off スイッチのみ — 実際のトークン不要）:

```
CLOUDFLARE_API_TOKEN=enabled
```
...
{
  "mcpServers": {
    "cloudflare-mcp": { "url": "http://127.0.0.1:8080/mcp/cloudflare" }
  }
}
```
...
MCP クライアントから `/mcp/cloudflare` にアクセスすると、ブラウザで Cloudflare の OAuth 同意画面が表示されます。
認可後はユーザートークンが `mcp-gateway-data` ボリューム（`/data/mcp-gateway/upstream_tokens.json`）に永続化され、以降は自動的に使用されます。

### 他の Remote MCP プロバイダーへの横展開

同じパターンで任意の upstream OAuth AS を持つ Remote MCP サービスを追加できます:

```
ROUTE_<NAME>=/<prefix>|<upstream_url>|auth=oauth|upstream_oauth=auto|upstream_oauth_scope=<scopes>
```

| オプション | 対象経路 | 説明 |
|---|---|---|
| `auth=oauth` | client → gateway | OAuth 認証を要求 |
| `upstream_oauth=auto` | gateway → upstream | RFC 9728/RFC 8414 で AS を自動検出し委任 |
| `upstream_oauth_scope=...` | gateway → upstream | upstream AS に要求するスコープ |

- DCR 登録結果は `upstream_clients.json`、ユーザートークンは `upstream_tokens.json` に永続化されます
- upstream が 401 を返した場合、トークンが削除され次回アクセス時に再認可フローが開始されます
