        ...
        try:
            ...
            if corrected_content != original_content:
                ...
                try:
                    ...
                    # 権限エラーの場合は/tmp配下にバックアップ作成
                    backup_path = Path(f'/tmp/{file_path.name}.bak_{timestamp}')
                    backup_path.write_text(original_content, encoding='utf-8')
                    logger.warning(f"⚠️ 権限エラーのため/tmpにバックアップ作成: {backup_path}")
...
class DateTimeValidatorHandler(FileSystemEventHandler):
    ...
    def on_modified(self, event):
        ...
        # バックアップファイルは無視
        if '.bak_' in file_path.name or file_path.suffix == '.bak':
            return
