...
crm-start: crm-stop ## copilot-review-mcp コンテナを起動（バックグラウンド、既存コンテナ自動削除）
	$(if $(GITHUB_CLIENT_ID),,$(error ERROR: GITHUB_CLIENT_ID is not set in .env or environment))
	$(if $(GITHUB_CLIENT_SECRET),,$(error ERROR: GITHUB_CLIENT_SECRET is not set in .env or environment))
	docker run -d \
		--name $(CRM_CONTAINER) \
...
crm-stop: ## copilot-review-mcp コンテナを停止・削除
	-docker stop $(CRM_CONTAINER)
	-docker rm   $(CRM_CONTAINER)
	@echo "stopped."
