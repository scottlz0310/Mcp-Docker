...
環境変数 (copilot-review-mcp):
  MCP_GATEWAY_URL               mcp-gateway の接続先 URL（最優先）
  MCP_GATEWAY_PUBLIC_URL        mcp-gateway の接続先 URL（MCP_GATEWAY_URL 未設定時のフォールバック）
  MCP_GATEWAY_BASE_URL          mcp-gateway の接続先 URL（互換エイリアス、PUBLIC_URL 未設定時のフォールバック）
...
環境変数 (mcp-gateway):
  MCP_GATEWAY_URL               HTTP 接続先 URL（最優先）
  MCP_GATEWAY_PUBLIC_URL        HTTP 接続先 URL（MCP_GATEWAY_URL 未設定時のフォールバック）
  MCP_GATEWAY_BASE_URL          HTTP 接続先 URL（互換エイリアス、PUBLIC_URL 未設定時のフォールバック）
