...
def check_docker() -> DiagnosticResult:
    ...
    try:
        ...
        return DiagnosticResult(
            ...
            details={"docker_path": docker_path, "error": str(e)}
        )
...
def check_act() -> DiagnosticResult:
    ...
    try:
        ...
        if result.returncode == 0:
            return DiagnosticResult(
                success=True,
                message="act available",
                details={"act_path": act_path, "version": result.stdout.strip()}
            )
        else:
            return DiagnosticResult(
                success=False,
                message="act command failed",
                details={"act_path": act_path, "error": result.stderr}
            )
    except Exception as e:
        return DiagnosticResult(
            success=False,
            message=f"act check failed: {e}",
            details={"act_path": act_path, "error": str(e)}
        )
