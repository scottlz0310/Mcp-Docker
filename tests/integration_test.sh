...
# 検証対象: GitHub MCP
# 目的: サービス起動確認
if docker-compose ps github | grep -q "Up"; then
    echo "✅ GitHub MCPサービス起動成功"
else
    echo "❌ GitHub MCPサービス起動失敗"
    exit 1
fi

echo "8. DateTime Validatorサービス確認"
# 検証対象: DateTime Validator
# 目的: Python環境・依存関係確認
if docker-compose ps datetime | grep -q "Up"; then
